...
## Querying from local agents

Enable **Settings → Routing → Agent Usage API** to let local agents such as
Claude Code and Codex query the usage already configured in CC Switch. The
service always listens on `127.0.0.1`, requires a Bearer token, and never
returns API keys, access keys, or secrets.

Query the active Claude provider:

```bash
curl -s \
  -H "Authorization: Bearer $CCSWITCH_AGENT_API_TOKEN" \
  "http://127.0.0.1:15722/_ccswitch/v1/usage?app=claude&provider=active"
```

List provider IDs for an app:

```bash
curl -s \
  -H "Authorization: Bearer $CCSWITCH_AGENT_API_TOKEN" \
  "http://127.0.0.1:15722/_ccswitch/v1/providers?app=claude"
```

Successful results are cached for 30 seconds. Add `&refresh=true` to bypass the
cache. The token is shown only when first generated or rotated; store it in a
local environment variable and never commit it to a repository.
