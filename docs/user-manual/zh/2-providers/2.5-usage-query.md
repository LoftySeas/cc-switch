...
在「设置 → 路由 → Agent 用量 API」中启用后，Claude Code、Codex 等本机
Agent 可以通过只读 HTTP 接口查询 CC Switch 已配置的用量。服务固定监听
`127.0.0.1`，并要求 Bearer Token；响应不会包含 API Key、AccessKey 或 Secret。
...
结果默认缓存 30 秒。需要跳过缓存时，在 URL 后添加 `&refresh=true`。Token 只在
首次生成或主动轮换时显示，请将其放入本机环境变量，不要写入项目仓库。
